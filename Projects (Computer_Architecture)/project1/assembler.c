...
int main(int argc, char *argv[]) {
	...
	while (read_and_parse(inFilePtr, &is_data_section,
		...
		if (is_data_section) {
			if (strlen(label)) {
				if (find_sym(dsym, n_dsym, label) != -1) {
			...
			if (strlen(label)) {
				if (find_sym(isym, n_isym, label) != -1) {
					printf("error: duplicate label '%s'\n", label);
					exit(1);
				}
	...
	/* Phase-2: generate machine codes
	   information is encoded into two sections; data[] and text[] */
	is_data_section = 0;
	while (read_and_parse(inFilePtr, &is_data_section,
				label, value, opcode, rd, rs, rt, imm)) {
		if (is_data_section) {
			...
			if (strlen(opcode)) {
				text[n_inst] = encode(n_inst, opcode, rs, rt, rd, imm);
				n_inst++;
			}
		}
	}

	/* Phase-3: write each section to outfile */
...
int gp_offset(struct symbol* symbols, int n_symbols, char* label) {
	...
	base = 0x10008000;
	idx = find_sym(symbols, n_symbols, label);
	if (idx == -1) {
		printf("error: undefined data label '%s'\n", label);
		exit(1);
	}
	// .data starts at 0x10000000
	return 0x10000000 + symbols[idx].offset * 4 - base;
}
...
int pc_offset(struct symbol* symbols, int n_symbols, char* label, int pc) {
	...
	// offset = (target - (PC+4)) / 4 = target_index - pc - 1
	return symbols[idx].offset - pc - 1;
}
...
int psuedo_offset(struct symbol* symbols, int n_symbols, char* label) {
	...
	// .text starts at 0x00400000, word address = (0x00400000 + offset*4) / 4
	return (0x00400000 / 4) + symbols[idx].offset;
}
...
unsigned int encode(int pc, char* opcode, char* rs, char* rt, char* rd, char* imm) {
	...
	if (!strcmp(opcode, "nop")) {
		...
		f_opcode = 0; f_rs = regnum(rs); f_rt = regnum(rt);
		f_rd = regnum(rd); f_func = 32;
	} else if (!strcmp(opcode, "slt")) {
		f_opcode = 0; f_rs = regnum(rs); f_rt = regnum(rt);
		f_rd = regnum(rd); f_func = 42;
	} else if (!strcmp(opcode, "nor")) {
		f_opcode = 0; f_rs = regnum(rs); f_rt = regnum(rt);
		f_rd = regnum(rd); f_func = 39;
	} else if (!strcmp(opcode, "jr")) {
		f_opcode = 0; f_rs = regnum(rs); f_func = 8;
	} else if (!strcmp(opcode, "addi")) {
		f_opcode = 8; f_rs = regnum(rs); f_rt = regnum(rt);
		f_imm = atoi(imm) & 0xFFFF;
	} else if (!strcmp(opcode, "lw")) {
		f_opcode = 35; f_rs = regnum(rs); f_rt = regnum(rt);
		f_imm = gp_offset(dsym, n_dsym, imm) & 0xFFFF;
	} else if (!strcmp(opcode, "sw")) {
		f_opcode = 43; f_rs = regnum(rs); f_rt = regnum(rt);
		f_imm = gp_offset(dsym, n_dsym, imm) & 0xFFFF;
	} else if (!strcmp(opcode, "beq")) {
		f_opcode = 4; f_rs = regnum(rs); f_rt = regnum(rt);
		f_imm = pc_offset(isym, n_isym, imm, pc) & 0xFFFF;
	} else if (!strcmp(opcode, "j")) {
		f_opcode = 2; f_imm = psuedo_offset(isym, n_isym, imm);
	} else if (!strcmp(opcode, "jal")) {
		f_opcode = 3; f_imm = psuedo_offset(isym, n_isym, imm);
	} else {
