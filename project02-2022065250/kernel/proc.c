      ...
      if(pp->parent == p && !pp->is_thread){
        ...
        if(pp->state == ZOMBIE){
          ...
          for (struct proc *cp = proc; cp < &proc[NPROC]; cp++) {
            if (cp->is_thread && cp->parent == pp && cp->state != ZOMBIE) {
              child_running = 1;
              break;
            }
          }
...
clone(void(*fcn)(void*,void*),void*arg1,void*arg2,void*stack){
	...
	np->trapframe_va = p->trapframe_va - PGSIZE * p->ChildThread;
	if(mappages(p->pagetable, np->trapframe_va, PGSIZE, (uint64)(np->trapframe), PTE_R | PTE_W) < 0)
	{
...
join(uint64 stack_addr)
{
  ...
  for (;;) {
    havekids = 0;
    for (p = proc; p < &proc[NPROC]; p++) {
      if (p->parent == main_thread && p->is_thread) {
        acquire(&p->lock);
        havekids = 1;
        ...
        if (p->state == ZOMBIE) {
          ...
          if (stack_addr != 0 && copyout(main_thread->pagetable, stack_addr, (char *)&p->ustack, sizeof(p->ustack)) < 0) {
            release(&p->lock);
            release(&wait_lock);
            return -1;
          }
    ...
    if (!havekids || killed(main_thread)) {
      release(&wait_lock);
      return -1;
    }

    sleep(main_thread, &wait_lock);
  }
}