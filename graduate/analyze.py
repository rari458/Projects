...
way that eyeballing does not catch. The paired difference was given a +/-0.05 error bar --
...
def load(path):
    """One CSV -> {arm: [(epoch, test_acc, time_s), ...]}, sorted by epoch.
    ...
    Handles both runlog layouts: the 5-column one every file written before 2026-08-11
    uses, and trhe 6-column one carrying `lr`. A kind that appears at more than one LR is
    splir into `kind@lr` arms -- an LR screen otherwise collapses three separate runs onto
    one name and reads as a single arm with impossible epoch numbers.
    """
    notes, body = [], []
    ...
    for r in csv.DictReader(body):
        ...
        rows[(kind, lr)].append((int(r["epoch"]), float(r["test_acc"]), float(r["time_s"])))
        lrs[kind].add(lr)
    arms = {(kind if len(lrs[kind]) == 1 else f"{kind}@{lr}"): sorted(hist)
            for (kind, lr), hist in rows.items()}
    return arms, notes

def last_n(hist, n):
    accs = [a for _, a, _ in hist[-n:]]
...
def at_budget(hist, budget, window):
    ...
    single-epoch reading, which takes one sample off a curve whose epoch-to jitter is
    ~0.3pp -- two independent sources of noise, one point of measurement.
    """
    within = [(e, a) for e, a, t in hist if t <= budget]
    if not within:
        return None
    tail = within[-window:]
...
def time_to(hist, target):
    ...
    The target is the other arm's *final* accuracy, not it best: `best` is itself a max
...
def compare(arms, a, b, last, window):
    ...
    ah, bh = arms[a], arms[b]
    out = {"final": ah[-1][1] - bh[-1][1],
           f"last{last}": last_n(ah, last)[0] - last_n(bh, last)[0]}
...
def pool(rows, pair, last, window):
    ...
    for key, unit in (("final", "pp"), (f"last{last}", "pp"), ("eq_single", "pp"),
        ...
        print(f"{key:<26}{statistics.mean(vals):>8.2f}{sd:>8}{len(vals):>4}   {shown}")
...
def main():
    ...
    ap.add_argument("--pair", nargs=2, metavar=("A", "B"), default=list(DEFAULT_PAIR),
                    help="the paired comparison to run in every file (default: muonsam muon)")
